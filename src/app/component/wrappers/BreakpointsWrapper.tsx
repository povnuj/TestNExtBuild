...
export default function BreakpointsWrapper({children}: Readonly<{children: React.ReactNode;}>) {
  ...
  useEffect(() => {
    let size = 'xl';
    const handleResize = () => {
      if(window.matchMedia('(min-width: 1604px)').matches && size !== 'xl' ) {
        uiContext.setState!(UiStatesType.setBreackpoint, 'xl');
        size = 'xl';
      }
      if(window.matchMedia('(min-width: 1266px) and (max-width: 1604px)').matches && size !== 'lg') {
        uiContext.setState!(UiStatesType.setBreackpoint, 'lg');
        size = 'lg';
      }

      if(window.matchMedia('(min-width: 1064px) and (max-width: 1266px)').matches && size !== 'md') {
        uiContext.setState!(UiStatesType.setBreackpoint, 'md');
        size = 'md';
      }

      if(window.matchMedia('(min-width: 575px) and (max-width: 1064px)').matches &&  size !== 'sm') {
