...
const PriceButtonGreenComponent = styled(Button,{
    name: 'PriceButtonGreenComponent',
    slot: 'root',
})(({}) => ({
    //color: Colors.main,
    'svg': {
    ...
    return (
        <Link href={props.buttonProps.url} style={{textDecoration: 'none'}}>
            <PriceButtonGreenComponent>
                <Box className={'s_green_btn_new_price'}>
                    {formattedPrice}
                ...
                <Box className={'s_green_btn_old_price'} sx={{fontSize: '22px', color: Colors.lightGray2, position: 'absolute', bottom: 0, left: '15px', textDecoration: `line-through ${Colors.light}` }}>
                    {formattedPrice}₴                    
